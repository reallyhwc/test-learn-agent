...
> 对应入口：[AgentApplication.java](../finance-agent/src/main/java/com/example/agent/AgentApplication.java)
...
> 文件：[application.yml](../finance-agent/src/main/resources/application.yml)
...
> 文件：[pom.xml](../finance-agent/pom.xml)
...
> 文件：[AccountContextBuilder.java](../finance-agent/src/main/java/com/example/agent/context/AccountContextBuilder.java)
...
> 对应文件：[ChatController.java](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java)
...
> 方法：[`chatStream()`](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L130-L262)
...
| 步骤 | 代码位置 | 说明 |
|------|---------|------|
| **禁用缓冲** | [L148-149](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L148-L149) | `setBufferSize(0)` + `X-Accel-Buffering: no`（Nginx 兼容） |
| **响应式订阅** | [L162-170](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L162-L170) | `chatClient.prompt().stream().chatResponse().subscribe(onNext, onError, onComplete)` |
| **SSE: 回复文本** | [L267-280](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L267-L280) | `writeSseData()` → 格式: `data:{text}\n\n` |
| **SSE: 推理过程** | [L282-294](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L282-L294) | `writeSseThinking()` → 格式: `event:thinking\ndata:{text}\n\n` |
| **SSE: 错误** | [L296-307](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L296-L307) | `writeSseError()` → 格式: `event:error\ndata:{msg}\n\n` |
| **TTFT 记录** | [L190-193](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L190-L193) | 首个非空 token 到达时记录 First Token Time |
| **断连检测** | [L267-307](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L267-L307) | `IOException`(Broken pipe) → `clientGone=true` → 跳过后续写入 |
| **Token 统计** | [L225-250](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L225-L250) | `onComplete` 中计算 TPS + 持久化到 `token-usage.jsonl` |

### 4.4 System Prompt 构建

> 方法：[`buildSystemPrompt()`](../finance-agent/src/main/java/com/example/agent/controller/ChatController.java#L360-L390)
...
> 对应文件：[FinanceTools.java](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java)
...
|--------|------|------|-----------------|------|
| `query_balance` | [`queryBalance()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L36-L57) | L36-57 | `GET /api/accounts/{id}/balance` | 查单个账户余额 |
| `list_transactions` | [`listTransactions()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L63-L148) | L63-148 | `GET /api/transactions?...` | 查交易明细列表 |
| `summarize_transactions` | [`summarizeTransactions()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L150-L193) | L150-193 | `GET /api/transactions/summary?...` | 按分类汇总交易 |
| `add_transaction` | [`addTransaction()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L208-L270) | L208-270 | `POST /api/transactions` | 添加一笔交易 |
| `list_accounts` | [`listAccounts()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L272-L298) | L272-298 | `GET /api/accounts?userId=` | 查全部账户 |
...
| 方法 | 行号 | 说明 |
|------|------|------|
| [`parseFilters()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L195-L206) | L195-206 | JSON 字符串 → Map，解析失败静默降级为空 Map |
| [`validateUserId()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L307-L316) | L307-316 | 正则校验 `[a-zA-Z0-9_-]{1,64}`，不合法则抛异常 |
| [`recordSuccess()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L318-L327) | L318-327 | Micrometer Counter + Timer |
| [`recordError()`](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java#L329-L343) | L329-343 | Micrometer Counter（按异常类型分 tag） |

### 5.5 MCP Server 配置

> 文件：[application.yml (MCP Server)](../finance-mcp-server/src/main/resources/application.yml)
...
| [AgentMetrics](../finance-agent/src/main/java/com/example/agent/metrics/AgentMetrics.java) | 全类 | — | Micrometer 指标 |
| [InputGuardrailAdvisor](../finance-agent/src/main/java/com/example/agent/guardrails/InputGuardrailAdvisor.java) | `before()` | L49-59 | Prompt Injection 检测 |
| [ToolCallGuardrailAdvisor](../finance-agent/src/main/java/com/example/agent/guardrails/ToolCallGuardrailAdvisor.java) | `before()` | L72-88 | 解析 userId 写入 context |
| ↑ | `after()` | L108-127 | 审计工具调用 |
| [OutputGuardrailAdvisor](../finance-agent/src/main/java/com/example/agent/guardrails/OutputGuardrailAdvisor.java) | `after()` | L55-90 | 金额提取 + 幻觉检测 |
| [PromptInjectionDetector](../finance-agent/src/main/java/com/example/agent/guardrails/PromptInjectionDetector.java) | `isInjection()` | L44-55 | 12 种注入模式正则检测 |

### MCP Server 模块 (`finance-mcp-server`)

| 文件 | 关键方法 | 行号 | 职责 |
|------|---------|------|------|
| [FinanceTools](../finance-mcp-server/src/main/java/com/example/mcp/tool/FinanceTools.java) | `queryBalance()` | L36-57 | 查余额 |
...
| 文件 | 关键配置 | 说明 |
|------|---------|------|
| [application.yml (Agent)](../finance-agent/src/main/resources/application.yml) | `spring.ai.openai.*` | LLM 连接配置 |
| ↑ | `spring.ai.mcp.client.*` | MCP Client 连接配置 |
| [application.yml (MCP)](../finance-mcp-server/src/main/resources/application.yml) | `spring.ai.mcp.server.*` | MCP Server 注册配置 |
| [.env.example](../.env.example) | `LLM_API_KEY` / `LLM_BASE_URL` / `LLM_MODEL` | LLM 凭证 |
...
> **提示**：本文档中所有链接使用相对路径格式，在 IntelliJ IDEA 的 Markdown 预览中按住 Ctrl/Cmd 点击即可跳转到对应文件。行号标注在文本中供参考。
