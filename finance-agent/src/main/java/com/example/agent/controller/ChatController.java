    ...
    private String buildSystemPrompt(String userId) {
        ...
        return """
                ...
                **关键规则：你必须通过调用工具来获取数据，绝对不能编造、猜测或使用自己的知识来回答财务问题。**
                ...
                图表输出：当回答包含数值统计时，用图表代码块替代 Markdown 表格。
                必须严格使用以下格式（三反引号 + chart:类型）：
                ...
                规则：
                - 第一行是图表标题，第二行是 CSV 列名，后续行是数据
                - 柱状图用 chart:bar，饼图用 chart:pie，折线图用 chart:line
                - 列名只写名称不写括号注释（"金额"不是"金额（元）"）
                - 数值只写数字，不带 ¥、元、逗号等符号
                - 代码块前后保留自然语言解释文字
